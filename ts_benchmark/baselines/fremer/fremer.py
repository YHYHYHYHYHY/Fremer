...
class Fremer(ModelBase):
    def __init__(self, **kwargs):
        super(Fremer, self).__init__()
    ...
    def forecast_fit(
        self, train_valid_data: pd.DataFrame, train_ratio_in_tv: float, series_name: str
    ) -> "ModelBase":
        """
        Train the model.
        ...
        self.model = FremerModel(self.config)
        ...
        path = os.path.join('./checkpoints', settings)
        ...
        for epoch in range(config.num_epochs):
            ...
            for i, (input, target, input_mark, target_mark) in enumerate(
                ...
                inference_time_list.append(inf1 - inf0)
                # training_time = min(training_time, t1 - t0)
                # inference_time = min(inference_time, inf1 - inf0)

            print('training loss: {:.6f}'.format(sum(train_loss)/len(train_loss)))
            if train_ratio_in_tv != 1:
                valid_loss = self.validate(valid_data_loader, criterion)
                self.early_stopping(valid_loss, self.model, path)
        ...
        inference_time = sum(inference_time_list) / len(inference_time_list)
        with open('efficiency.txt', 'a') as f:
            f.write(settings+'\n')
            f.write("Training:{} Inference:{} total_params:{}\n\n".format(training_time, inference_time, total_params))
